...
function getApiKey(): string {
  ...
  if (!key) throw new Error("LOCALISE_API_KEY environment variable is not set");
  return key;
}

async function locoFetch(
  endpoint: string,
  options: RequestInit & { rawBody?: boolean; formBody?: Record<string, string> } = {}
): Promise<unknown> {
...
server.tool(
  "list_locales",
  "List all locales configured in the localise.biz project",
  {},
  async () => {
    const locales = (await locoFetch("/locales")) as Array<{
      code: string;
      name: string;
      plurals: { length: number };
    }>;
    const list = locales
      .map((l) => `${l.code} — ${l.name}`)
      .join("\n");
...
server.tool(
  "list_assets",
  "List all translation keys (assets) in the project",
  {
    filter: z
      .string()
      .optional()
  ...
  async ({ filter }) => {
    ...
    const assets = (await locoFetch(endpoint)) as Array<{
      id: string;
      type: string;
      context: string;
    }>;
...
server.tool(
  "create_asset",
  "Create a new translation key (asset) in localise.biz",
  {
...
server.tool(
  "get_translations",
  "Get all translations for a specific asset/key across all locales",
  {
  ...
  async ({ assetId }) => {
    ...
    )) as Record<string, { translation: string; status: string }>;
...
server.tool(
  ...
  async ({ locale, format }) => {
    ...
    const result = await locoFetch(
      `/export/locale/${encodeURIComponent(locale)}.${ext}`
    );
    const output =
...
server.tool(
  "batch_translate",
  "Create an asset and add translations for multiple locales at once",
  {
    ...
    translations: z
      .record(z.string())
      .describe(
        "Object mapping locale codes to translated text, e.g. { \"fr\": \"Soumettre\", \"de\": \"Einreichen\" }"
      ),
    context: z.string().optional().describe("Context for translators"),
  },
  async ({ id, sourceText, translations, context }) => {
    // Step 1: Create the asset
    const body: Record<string, string> = { id, text: sourceText };
    if (context) body.context = context;
    ...
    const results: string[] = [`Asset "${id}" created with source: "${sourceText}"`];
    for (const [locale, text] of Object.entries(translations)) {
      await locoFetch(
        `/translations/${encodeURIComponent(id)}/${encodeURIComponent(locale)}`,
        { method: "POST", body: text, rawBody: true }
      );
